...
def verificarEscopo(tabelaDeTokens, posicaoToken):
    try:
        ...
        mensagemErro("❌ Erro: Excecao na verificacao de escopo. Ops! Parece que algo deu errado por aqui… 😭")
    ...
    try:
        ...
        for i in range(posicao-1):
            if(nomeProcedimento == lexemas[i]):
                if(lexemas[i-1] == 'procedimento'): # Verificando se eh declaracao ou chamada
                    declaradoAnteriormente = True
        
        if not declaradoAnteriormente:
            mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(numeroLinhas[posicao]) + ". Procedimento " + str(lexemas[posicao-1][0]) + " nao declarado anteriormente.")         

    except IndexError:
        mensagemErro("❌ Erro: Excecao na verificacao de procedimento. Ops! Parece que algo deu errado por aqui… 😭")
    ...
    try:
        ...
        for i in range(posicao - 1):
            if nomeFuncao == lexemas[i]:
                if lexemas[i - 2] == 'funcao': # Verificando se eh declaracao ou chamada
                    declaradaAnteriormente = True
                    tipoFuncao = lexemas[i - 1]
                    
        if not declaradaAnteriormente:
            mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(numeroLinhas[posicao]) + ". Funcao " + str(lexemas[posicao - 1][0]) + " nao declarada anteriormente.")         
        
        if tipoFuncao != tipoVariavel:
            mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(numeroLinhas[posicao][0]) + ". Tipo de variavel " + str(lexemas[posicao - 3][0]) + " diferente do retorno da funcao.")

    except IndexError:
        mensagemErro("❌ Erro: Excecao na verificacao de funcao. Ops! Parece que algo deu errado por aqui… 😭")
    ...
    try:
        ...
        if token == "constante" and tipoFuncao != 'int':
            ...
            if indiceDeclaracaoVariavel == posicao: # Variavel so existe na linha de retorno
                mensagemErro("❌ Erro: Ocorreu um erro semantico no retorno da funcao, na linha " + str(numeroLinhas[posicao][0]) + ". Variavel " + nomeVariavel + " nao declarada anteriormente.")         
        ...
        mensagemErro("❌ Erro: Ocorreu uma excecao na verificacao do tipo de retorno da funcao. Ops! Parece que algo deu errado por aqui… 😭 ")
...
def verificarSeVariavelEhBooleana(posicao, lexemas, numeroLinhas):
    ...
    if lexemas[posicaoDoTipo] != 'boolean':
        mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(numeroLinhas[posicao][0]) + 
...
def verificarSeVariavelExisteETiposSaoIguais(posicao, tokens, lexemas, numeroLinhas):
    ...
    if not declaradaAnteriormente:
        mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(numeroLinhas[posicao][0]) + ". Variavel " + nomeVariavel + " nao declarada anteriormente.")         

    if tipo != tipoAtribuido:
        mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(numeroLinhas[posicao][0]) + ". Variavel " + nomeVariavel + " possui tipo diferente do esperado.")         
...
def verificarSeVariavelExiste(posicao, tokens, lexemas, numeroLinha):
    ...
    if not declaradaAnteriormente:
        mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + numeroLinha + ". Variavel " + nomeVariavel + " nao declarada anteriormente.")         
    ...
    for posicao in range(tamanhoDaTabela):
        if tabelaDeSimbolos['Valor'][posicao][0] == 'f': # Toda funcao comeca com f, logo a chamada dela tambem
            ...
            if qtdArgumentos != qtdParametros:
                mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(tabelaDeSimbolos['Linha'][posicao]) + ". Numero de argumentos diferente do esperado pela funcao " + nomeDoMetodo + ". Esperava-se " 
            ...
            for indice in range(qtdParametros):
                ...
                if tabelaDeSimbolos['Lexema'][indiceLexema] != nomeVariavel:
                    mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(tabelaDeSimbolos['Linha'][posicao]) + ". Variavel " + nomeVariavel + " nao declarada anteriormente.")

                if tabelaDeSimbolos['Tipo'][indiceLexema] != tiposVariaveis[indice]:
                    mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(tabelaDeSimbolos['Linha'][posicao]) + 
...
def verificarTipoDeParametroEArgumentoDeProcedimento(tabelaDeSimbolos):
    ...
    for posicao in range(tamanhoDaTabela):
        if tabelaDeSimbolos['Token'][posicao] == 'idProcedimento':
            ...
            qtdParametros = len(tiposVariaveis)
            if qtdArgumentos != qtdParametros:
                mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(tabelaDeSimbolos['Linha'][posicao]) + ". Numero de argumentos diferente do esperado pelo procedimento " + nomeDoMetodo + ". Esperava-se " 
                + str(qtdParametros) + ", porem ela recebeu " + str(qtdArgumentos))
...
def compararTipos(tokens, lexemas, numeroLinha, posicao, operador, posicao2):
    ...
    if tipo1 != tipo2:
        mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(numeroLinha[0]) + ". Tipo " + tipo1 + " sendo comparado ou operado com " + tipo2)
    
    if tipo1 == 'boolean':
        if operador not in ['==', '!=']:
            mensagemErro("❌ Erro: Ocorreu um erro semantico na linha " + str(numeroLinha[0]) + ". Operador " + operador + " invalido para tipo booleano")
