    ...
    for linha, coluna in tabelaDeSimbolos.iterrows():
        if(coluna['Token'] == 'funcao'):
            if(not coluna['Lexema'] in nomesFuncoes):
                ...
                mensagemErro("❌ Erro: Ocorreu um erro semantico na linha "+ str(coluna['Linha']) + ". Ja existe uma funcao chamada " + coluna['Lexema'])
    ...
    for linha, coluna in tabelaDeSimbolos.iterrows():
        if(coluna['Token'] == 'procedimento'):
            if(not coluna['Lexema'] in nomesProcedimento):
                ...
                mensagemErro("❌ Erro: Ocorreu um erro semantico na linha "+ str(coluna['Linha']) + ". Ja existe um procedimento chamado " + coluna['Lexema'])
            ...
            if coluna['Token'] == 'idVariavel':
                if coluna['Escopo'] == escopo:
                    if not coluna['Lexema'] in variaveis:
                        ...
                        mensagemErro("❌ Erro: Ocorreu um erro semantico na linha "+ str(coluna['Linha']) + ". A variavel " + coluna['Lexema'] + " ja foi declarada neste escopo ")
